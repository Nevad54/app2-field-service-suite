...
const equipment = [
 { id: 'EQP-001', name: 'Carrier Rooftop Unit RTU-1', type: 'HVAC', customer_id: 'CUST-001', location: 'Building A - Roof', serial_number: 'CR-2024-001', install_date: '2024-06-15', status: 'operational', notes: 'Primary building HVAC unit', created_at: '2026-01-05T08:00:00Z' },
 { id: 'EQP-002', name: 'Generator GEN-500', type: 'Electrical', customer_id: 'CUST-003', location: 'Warehouse North', serial_number: 'GN-2023-042', install_date: '2023-11-20', status: 'operational', notes: '500kW backup generator', created_at: '2026-01-10T09:00:00Z' },
 { id: 'EQP-003', name: 'Boiler BLR-200', type: 'Plumbing', customer_id: 'CUST-004', location: 'Mechanical Room', serial_number: 'BL-2022-015', install_date: '2022-08-10', status: 'operational', notes: 'Main hospital boiler', created_at: '2026-01-15T10:00:00Z' },
 { id: 'EQP-004', name: 'Chiller CH-1000', type: 'HVAC', customer_id: 'CUST-006', location: 'Hotel Basement', serial_number: 'CH-2021-008', install_date: '2021-03-25', status: 'needs_maintenance', notes: 'Main cooling system - annual service due', created_at: '2026-01-20T11:00:00Z' },
 { id: 'EQP-005', name: 'Elevator ELV-3', type: 'General', customer_id: 'CUST-006', location: 'Main Building', serial_number: 'ELV-2020-003', install_date: '2020-01-15', status: 'operational', notes: 'Passenger elevator', created_at: '2026-02-01T08:00:00Z' },
 { id: 'EQP-006', name: 'Security System SEC-HQ', type: 'Security', customer_id: 'CUST-005', location: 'District Office', serial_number: 'SEC-2023-101', install_date: '2023-09-01', status: 'operational', notes: 'Headquarters access control', created_at: '2026-02-05T14:00:00Z' },
];

const quotes = [
 { id: 'QUO-2026-001', customer_id: 'CUST-001', title: 'HVAC Upgrade Proposal', description: 'Complete HVAC system upgrade for Building A', status: 'pending', total_amount: 45000.00, valid_until: '2026-03-15', created_by: 'admin', created_at: '2026-02-01T10:00:00Z', items_json: JSON.stringify([{ description: 'New RTU installation', quantity: 1, unit_price: 35000 }, { description: 'Ductwork modifications', quantity: 1, unit_price: 7500 }, { description: 'Electrical upgrades', quantity: 1, unit_price: 2500 }]) },
 { id: 'QUO-2026-002', customer_id: 'CUST-002', title: 'Preventive Maintenance Contract', description: 'Annual PM contract for all systems', status: 'accepted', total_amount: 12000.00, valid_until: '2026-02-28', created_by: 'admin', created_at: '2026-02-10T09:00:00Z', accepted_at: '2026-02-12T14:00:00Z', job_id: null, items_json: JSON.stringify([{ description: 'Quarterly HVAC maintenance', quantity: 4, unit_price: 2000 }, { description: 'Emergency service call', quantity: 2, unit_price: 150 }, { description: 'Filter replacement', quantity: 12, unit_price: 150 }]) },
 { id: 'QUO-2026-003', customer_id: 'CUST-004', title: 'Boiler Replacement Estimate', description: 'Replace aging boiler system', status: 'pending', total_amount: 85000.00, valid_until: '2026-04-01', created_by: 'admin', created_at: '2026-02-15T11:00:00Z', items_json: JSON.stringify([{ description: 'New boiler unit', quantity: 1, unit_price: 65000 }, { description: 'Piping and installation', quantity: 1, unit_price: 15000 }, { description: 'Permits and inspections', quantity: 1, unit_price: 5000 }]) },
];

const recurringJobs = [
 { id: 'REC-001', customer_id: 'CUST-001', title: 'Quarterly HVAC Maintenance', description: 'Preventive maintenance for all HVAC units', frequency: 'quarterly', interval_value: 3, interval_unit: 'months', start_date: '2026-01-15', end_date: '2026-12-31', status: 'active', assigned_to: 'technician', category: 'maintenance', priority: 'medium', estimated_duration_hours: 4, created_by: 'admin', created_at: '2026-01-05T08:00:00Z' },
 { id: 'REC-002', customer_id: 'CUST-003', title: 'Monthly Generator Test', description: 'Monthly generator load test and inspection', frequency: 'monthly', interval_value: 1, interval_unit: 'months', start_date: '2026-01-01', end_date: '2026-12-31', status: 'active', assigned_to: 'technician', category: 'inspection', priority: 'high', estimated_duration_hours: 2, created_by: 'admin', created_at: '2026-01-10T09:00:00Z' },
 { id: 'REC-003', customer_id: 'CUST-006', title: 'Weekly Elevator Inspection', description: 'Weekly safety inspection and test', frequency: 'weekly', interval_value: 1, interval_unit: 'weeks', start_date: '2026-01-01', end_date: null, status: 'active', assigned_to: 'technician', category: 'inspection', priority: 'high', estimated_duration_hours: 1, created_by: 'admin', created_at: '2026-01-15T10:00:00Z' },
 { id: 'REC-004', customer_id: 'CUST-005', title: 'Annual Fire Safety Inspection', description: 'Yearly fire system inspection', frequency: 'yearly', interval_value: 1, interval_unit: 'years', start_date: '2026-06-01', end_date: '2026-06-30', status: 'scheduled', assigned_to: 'technician', category: 'safety', priority: 'high', estimated_duration_hours: 8, created_by: 'admin', created_at: '2026-02-01T08:00:00Z' },
];
...
async function seed() {
    ...
    const createTablesSQL = `
        CREATE TABLE IF NOT EXISTS inventory (
            ...
            quantity INTEGER DEFAULT 0, unit_price NUMERIC DEFAULT 0, reorder_level INTEGER DEFAULT 5,
            location TEXT, supplier TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
        );
        CREATE TABLE IF NOT EXISTS equipment (
            id TEXT PRIMARY KEY, name TEXT NOT NULL, type TEXT, customer_id TEXT,
            location TEXT, serial_number TEXT, install_date TEXT, status TEXT DEFAULT 'operational',
            notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
        );
        CREATE TABLE IF NOT EXISTS quotes (
            id TEXT PRIMARY KEY, customer_id TEXT, title TEXT NOT NULL, description TEXT,
            status TEXT DEFAULT 'pending', total_amount NUMERIC DEFAULT 0, valid_until TEXT,
            created_by TEXT, accepted_at TIMESTAMPTZ, job_id TEXT, items_json JSONB,
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        CREATE TABLE IF NOT EXISTS recurring_jobs (
            id TEXT PRIMARY KEY, customer_id TEXT, title TEXT NOT NULL, description TEXT,
            frequency TEXT, interval_value INTEGER DEFAULT 1, interval_unit TEXT DEFAULT 'months',
            start_date TEXT, end_date TEXT, status TEXT DEFAULT 'active', assigned_to TEXT,
            category TEXT, priority TEXT DEFAULT 'medium', estimated_duration_hours INTEGER DEFAULT 1,
            created_by TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
        );
        CREATE TABLE IF NOT EXISTS technicians (
            id TEXT PRIMARY KEY, name TEXT NOT NULL, email TEXT, phone TEXT,
            role TEXT DEFAULT 'technician', skills JSONB DEFAULT '[]'::jsonb, hourly_rate NUMERIC DEFAULT 0,
            certifications JSONB DEFAULT '[]'::jsonb, availability JSONB, status TEXT DEFAULT 'active',
            color TEXT, hire_date TEXT, notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
        );
    `;
    ...
    for (const j of jobs) {
        await supabase.from('jobs').upsert({
            ...
            assigned_to: j.assignedTo, location: j.location, customer_id: j.customerId,
            scheduled_date: j.scheduledDate, category: j.category, notes: j.notes,
    ...
    for (const i of invoices) {
        await supabase.from('invoices').upsert({
            id: i.id, job_id: i.jobId, customer_id: i.customerId, amount: i.amount,
            status: i.status, issued_date: i.issuedDate, paid_date: i.paidDate,
    ...
    for (const act of activityLogs) {
        await supabase.from('activity_logs').upsert(act);
    }
    ...
    for (const eq of equipment) {
        await supabase.from('equipment').upsert(eq);
    }
    ...
    for (const rec of recurringJobs) {
        await supabase.from('recurring_jobs').upsert(rec);
    }

    console.log("Seeding technicians...");
    for (const tech of technicians) {
        await supabase.from('technicians').upsert({
            id: tech.id, name: tech.name, email: tech.email, phone: tech.phone,
            role: tech.role, skills: tech.skills, hourly_rate: tech.hourly_rate,
            certifications: tech.certifications, availability: tech.availability,
            status: tech.status, color: tech.color, hire_date: tech.hire_date, notes: tech.notes
        });
    }
